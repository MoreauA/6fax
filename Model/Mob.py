...
class Tomate(Monster):
    ...
    def __init__(self, wall):
        Monster.__init__(self, self.VALUE, self.MAXLIFE, [50, 75], 0, self.SPEED, wall)
        self.state = 0
        self.animation = 0
        if wall==1:
            self.right = True
...
class Player(Mob):
    ...
    def __init__(self, initPos, initSize, initForce):
        Mob.__init__(self, initPos, self.MAXLIFE, initSize, initForce, [4, 4])
        self.gravitation = [0, 7]
        self.shots = []
        self.push = [0, 0]
        self.wall = 1
        self.precShoot = time.time()
